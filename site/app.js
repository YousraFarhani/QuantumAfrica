...
<section class="sec"><div class="wrap">
  <div class="split">
    ...
    <div class="panel">
      ...
      <div class="field"><label>Country</label><select><option>Select…</option>${CHAPTERS.map(c=>`<option>${esc(c.name)}</option>`).join('')}<option>Other</option></select></div>
      <button class="btn teal" type="button">Subscribe <span class="ar" aria-hidden="true">&rarr;</span></button>
      <p class="xs mt16">Connect this to your existing newsletter tool. No data is collected by this prototype.</p>
    </div>
  </div>
</div></section>`;
...
<section class="sec"><div class="wrap"><div class="side">
  <div>
    ...
    <p class="small mt8">This form is a placeholder. In production it posts to a route handler and writes into the CMS or your existing membership tool.</p>
    <div class="mt24">
      <div class="grid g2">
        <div class="field"><label>Full name</label><input placeholder="Your name"></div>
        <div class="field"><label>Email</label><input type="email" placeholder="you@university.edu"></div>
        <div class="field"><label>Country</label><select><option>Select…</option>${CHAPTERS.map(c=>`<option>${esc(c.name)}</option>`).join('')}<option>Other</option></select></div>
        <div class="field"><label>Institution</label><input placeholder="University or company"></div>
      </div>
      <div class="field"><label>I am a…</label><select><option>Student</option><option>Researcher</option><option>Educator</option><option>Industry professional</option><option>Other</option></select></div>
      <div class="field"><label>What are you looking for?</label><textarea rows="4" placeholder="Learning, research, mentorship, opportunities, starting a chapter…"></textarea></div>
      <button class="btn teal" type="button">Join Quantum Africa <span class="ar" aria-hidden="true">→</span></button>
    </div>
  </div>
  <aside>
...
<section class="sec"><div class="wrap"><div class="side">
  <div>
    ...
    <div class="field"><label>Reason</label><select><option>Partnership</option><option>University collaboration</option><option>Media</option><option>Speaking request</option><option>Submit an opportunity</option><option>Other</option></select></div>
    <div class="field"><label>Message</label><textarea rows="6"></textarea></div>
    <button class="btn teal" type="button">Send message</button>
  </div>
  <aside><div class="panel"><h5>Direct</h5><dl class="dl-list">
    <div class="dl-item"><dt>Email</dt><dd>${pht('email address')}</dd></div>
    <div class="dl-item"><dt>LinkedIn</dt><dd>${pht('URL')}</dd></div>
    <div class="dl-item"><dt>Based in</dt><dd>${pht('City, country')}</dd></div>
  </dl></div></aside>
</div></div></section>`;
...
function initSlider(){
  ...
  /* A video slide holds the stage until it ends; a photo slide gets six
     seconds. Videos are muted and inline, which is what lets them play at all
     on a phone, and they never play when the visitor has asked for less
     motion. */
  function stopAll(){
  ...
  function play(){
    ...
    slideTimer = setInterval(()=>{ go(slideIdx+1); play(); }, isVideo ? 20000 : 6000);
  }
